...
func Heuristic(stateChess []interface{}, p1 string, p2 string) ([3][3]string, int) {
	max := 0
	lastState := stateChess[0]
	for i := 0; i < len(stateChess); i++ {
		h := CheckNgang(stateChess[i].([3][3]string), p1, p2)
		if CheckDoc(stateChess[i].([3][3]string), p1, p2) > h {
			h = CheckDoc(stateChess[i].([3][3]string), p1, p2)
		}
		if CheckCheoDuoi(stateChess[i].([3][3]string), p1, p2) > h {
			h = CheckCheoDuoi(stateChess[i].([3][3]string), p1, p2)
		}
		if CheckCheoTren(stateChess[i].([3][3]string), p1, p2) > h {
			h = CheckCheoTren(stateChess[i].([3][3]string), p1, p2)
		}
		if h > max {
			max = h
			lastState = stateChess[i]
		}
	}

	return lastState.([3][3]string), max
}

func CheckNgang(tableChess [3][3]string, x string, o string) int {
	temp := []int{}
	max := 0
	for i := 0; i < len(tableChess); i++ {
		h := 0
...
func CheckDoc(tableChess [3][3]string, x string, o string) int {
	temp := []int{}
	max := 0
	for i := 0; i < len(tableChess); i++ {
		h := 0
...
func CheckCheoDuoi(tableChess [3][3]string, x string, o string) int {
	max := 0
	for i := 0; i < len(tableChess); i++ {
		if tableChess[i][i] == x {
			max += 1
...
func CheckCheoTren(tableChess [3][3]string, x string, o string) int {
	max := 0
	for i := 0; i < len(tableChess); i++ {
		if tableChess[i][len(tableChess)-i-1] == x {
			max += 1
		}
		if tableChess[i][len(tableChess)-i-1] == o {
			max = 0
		}
	}
	return max
